...
URL="https://poloniex.com/public?command=returnChartData&currencyPair=BTC_DASH&start=1105699200&end=9999999999&period=7200"


with open('dash_11_10_18.json') as f:
...
high_prices = dataset[:,0]
low_prices = dataset[:,1]
mid_prices = (high_prices+low_prices)/2.0

split = round(mid_prices.size * 1)

train_data = mid_prices[:split]
test_data = mid_prices[split:]

plt.plot(range(train_data.size),train_data, label = "Original mid")
...
plt.plot(range(train_data.size),train_data, label = "Scaled by windows")
...
plt.plot(range(train_data.size),train_data, label = "EMA")
plt.legend()
plt.show()
...
test_points_seq = np.arange(train_data.size-2000-1,train_data.size,50).tolist()
...
for ep in range(epochs):
    ...
    for step in range(train_seq_length//batch_size):
        ...
        average_loss += l
        tf.train.Saver().save(session, '/ml/regression/tfsaves/epoch_'+str(ep+1))
    # ============================ Validation ==============================
...
for p_i,p in enumerate(predictions_over_time[::3]):
    for xval,yval in zip(x_axis_seq,p):
        if p_i == 7 :
            plt.plot(xval,yval,color='r',alpha=alpha[p_i], label="Epoch "+str(p_i))
        else:
